...
DEFAULT_CSV = Path(__file__).with_name("BaseMN_003.csv")
...
with st.sidebar:
    ...
    image_root = st.text_input(
        "Pasta local das imagens (opcional)",
        placeholder=r"MN_003",
        help="Usada para exibir a imagem quando o CSV possui apenas o nome do arquivo.",
    )
...
with catalog_tab:
    ...
    catalog_columns = existing_columns(
        ...
        [
            ...
            "catalogo_estado_de_conservação", "status_cruzamento",
        ],
    )
    st.dataframe(
